...
import { ArrowLeft, ArrowRight, MapPin, Languages, Tag } from 'lucide-react';

const ExpertProfile = () => {
  const { t, isRTL, language } = useLanguage();
  const navigate = useNavigate();
  const { profile, loading, notFound } = useProfileDetail('expert');
  ...
  if (notFound) return (
    <Layout>
      <div className="container mx-auto px-4 py-20 text-center">
        ...
        <Button asChild><Link to="/experts"><BackIcon className={`w-4 h-4 ${isRTL ? 'ml-2' : 'mr-2'}`} />{isRTL ? 'العودة للخبراء' : 'Back to Experts'}</Link></Button>
      </div>
    </Layout>
  );
    ...
    <Layout>
      <div className="container mx-auto px-4 py-8">
        <Button variant="ghost" asChild className="mb-6">
          <Link to="/experts"><BackIcon className={`w-4 h-4 ${isRTL ? 'ml-2' : 'mr-2'}`} />{isRTL ? 'العودة للخبراء' : 'Back to Experts'}</Link>
        </Button>
